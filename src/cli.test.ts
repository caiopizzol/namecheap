...
const temporaryDirectory = mkdtempSync(join(tmpdir(), "namecheap-cli-"));
const preload = join(temporaryDirectory, "fetch.mjs");
...
beforeAll(() => {
  ...
  if (process.env.MOCK_ERROR) throw new Error("upstream unavailable");
  if (new URL(url).hostname !== "api.sandbox.namecheap.com") {
    throw new Error("expected sandbox");
  }
  return new Response('<ApiResponse Status="OK"><CommandResponse><DomainCheckResult Domain="example.com" Available="false" ErrorNo="0" IsPremiumName="false" /></CommandResponse></ApiResponse>');
};`,
	);
});
...
it("returns API failures on stderr", () => {
	const result = run(["check", "example.com"], true, true);
	expect(result.status).toBe(1);
	expect(result.stdout).toBe("");
